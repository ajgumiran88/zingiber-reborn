 ...
 * @extends WC_Widget
 */
class Vonaco_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'vonaco');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'vonaco');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'vonaco'),
				'label' => esc_html__('Title', 'vonaco'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'vonaco'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'vonaco'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'vonaco'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'vonaco'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'vonaco'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'vonaco'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'vonaco'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Vonaco_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'vonaco');
			...
			echo '<ul class="product-brands" id="vonaco-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('vonaco_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'vonaco'),
		'singular_name'              => esc_html__('Brands', 'vonaco'),
		'menu_name'                  => esc_html__('Brands', 'vonaco'),
		'all_items'                  => esc_html__('All Brands', 'vonaco'),
		'parent_item'                => esc_html__('Parent Brand', 'vonaco'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'vonaco'),
		'new_item_name'              => esc_html__('New Brand Name', 'vonaco'),
		'add_new_item'               => esc_html__('Add New Brands', 'vonaco'),
		'edit_item'                  => esc_html__('Edit Brand', 'vonaco'),
		'update_item'                => esc_html__('Update Brand', 'vonaco'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'vonaco'),
		'search_items'               => esc_html__('Search Brands', 'vonaco'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'vonaco'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'vonaco'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
